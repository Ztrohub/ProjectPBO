        ...
        for (int i = 0; i < 3; i++){
            for (int j = 0; j < 3; j++){
                if (cards[i][j] == null){
                    ...
                    if (enemyorhelp > (Math.min(step * 4, 35))){
                        if (rand < 25) cards[i][j] = new Shield();
                        else if (rand < 50) cards[i][j] = new Sword();
                        else if (rand < 65) cards[i][j] = new Gold();
                        else cards[i][j] = new Healing();
                    } else {
                        int stage = step/10 + 1;
                        if (rand < 50) cards[i][j] = new Slime(stage);
                        else if (rand < 70) cards[i][j] = new Mummy(stage);
                        else if (rand < 80) cards[i][j] = new Poison();
                        else {
